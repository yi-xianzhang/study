package com.yixian.basics.type.wrapper.booleans.synchronize;
...
public class AtomicBooleanDemo extends AbstractTestTemplate<AtomicBoolean> {
	...
	protected void instanceMethodsOfThis() throws Exception {
		...
		System.out.println(falseAtomicBoolean.toString() +
			...
			falseAtomicBoolean.compareAndSet(false, true) +
				", 当前值：" + falseAtomicBoolean);

		falseAtomicBoolean = new AtomicBoolean(false);
		System.out.println(falseAtomicBoolean.toString() +
				".compareAndSet([boolean]false,[boolean]false) : " +
			falseAtomicBoolean.compareAndSet(false, false) +
				", 当前值：" + falseAtomicBoolean);


		System.out.println(trueAtomicBoolean.toString() +
				".compareAndSet([boolean]false,[boolean]false) : " +
			trueAtomicBoolean.compareAndSet(false, false) +
				", 当前值：" + trueAtomicBoolean);

		System.out.println(trueAtomicBoolean.toString() +
				".compareAndSet([boolean]true,[boolean]false) : " +
			trueAtomicBoolean.compareAndSet(true, false) +
				", 当前值：" + trueAtomicBoolean);
		...
		System.out.println(trueAtomicBoolean.toString() +
				".compareAndSet([boolean]true,[boolean]true) : " +
			trueAtomicBoolean.compareAndSet(true, true) +
				", 当前值：" + trueAtomicBoolean);
		 ...
		 */
		System.out.println(trueAtomicBoolean.toString() +
				".getAndSet([boolean]false) : " +
			trueAtomicBoolean.getAndSet(false) +
				", 当前值：" + trueAtomicBoolean);
	...
	public static boolean staticMethodsFlag() {
		...
		for (Method method : declaredMethods) {
			...
			if (method.toString().contains(
					MethodType.STATIC_METHOD_TYPE.getName())) {
				return true;
			}
		}
		return false;
	...
	public static boolean constructorsFlag() {
		...
		for (Constructor constructor : constructors) {
			if (constructor == null || StringUtils.isEmpty(
					constructor.toString())) {
				continue;
			}
			if (constructor.toString().contains(
					MethodType.PUBLIC_METHOD_TYPE.getName())) {
				return true;
			}
		}
		return false;
